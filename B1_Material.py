...
class Material: 
    ...
    def __init__(self, concrete_class: str, steel_class: str,
        ...
        '''Args:
            ...
            steel_class (string): defined by user
            prestress_name (string): defined by user, according to table 2 in EN10138-3
            prestress_diameter (float): defined by user, according to table 2 in EN10138-3
        Returns:
            All factors and parameters needed to calculate SLS and ULS capacity for a simply 
            supported beam with evenly distributed load. 
        '''
    ...
    # STEEL PARAMTERS
        ...
        self.Ep = self.get_Ep()
        self.index_name = self.get_index_prestress(prestress_name)
        self.index_prestress = self.get_new_index(prestress_diameter,prestress_name)   
       ...
       # Design prestress strength according to EC2 3.3.6(6)
        self.fpd: float = self.fp01k / self.gamma_steel
...
# ----------------------STEEL PARAMETERS---------------------------------------------------------------
    ...
    def get_index_prestress(self,prestress_name:str)-> int: 
        ...
        match prestress_name:
            ...
            case 'Y1960S7':
                return 22
            case _:
                print("There is no steel name called", prestress_name)
                sys.exit("Script terminated due to an error.")

    def get_new_index(self,prestress_diameter:float ,prestress_name:str ):
        '''Get index based on given indexes and diameters of prestressing material from table 2 in EN10138-3.
        The reason for doing this is that one steel type comes with different diameters. 
        Args: 
            prestress_name(string): defined by user
            prestress_diameter(float): defined by user
        Returns:
            new and corrected index for determining parameters for prestress
        '''
    ...
    def get_Fpk(self)-> float:
        ...
        '''
        Fpk = [26.6,39.2,43.5,54,56,93,140,173,186,260,279,248,265,354,209,300,380,29.4,
              28,41.4,60.9,62,98]
        return Fpk[self.index_prestress]
    ...
    def get_Fp01k(self)-> float:
        ...
        '''
        Fp01k = [22.9,33.8,37.4,46.4,48,80,120,149,160,224,240,213,228,304,180,258,327,
                26.2,24.1,35.6,52.4,53,84]
    ...
    def calculate_fp01k(self,Fp01k:float ,Ap:float)-> float:
        ...
        Args:
            ...
            Ap(float): cross sectional area for prestress [mm2]
        Returns:
            fp01k(float): characteristic 0.1% proof force [N/mm2]
        '''
        fp01k = Fp01k * 10 ** 3/ Ap
        return fp01k
