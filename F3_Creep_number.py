...
class Creep_number:
    ...
    def __init__(self, cross_section, material, t0_self: int, t0_live: int, RH: int, cement_class: int,
        ...
        '''Args: 
            ...
            t0_self(int):  time of applied self load [days]
            t0_live(int):  time of applied live load [days]
        ...
        self.phi_self = self.calcualte_phi(self.phi_0_self,self.beta_c)
        self.phi_live = self.calcualte_phi(self.phi_0_self,self.beta_c)
    ...
    def calculate_beta_t0(self,t0_adjusted: float)-> float: 
        ...
        Args:
            t0_adjusted(float):  adjusted application age because of effect from cement type [days]
        Returns:
            beta_t0(float):  factor that take into acount the effect of concrete age 
        when load is applied 
        '''
