...
class Crack_control:
    ''' Class to contain crack control in Service limit state (SLS)
    ...
    '''
    def __init__(self,cross_section,load,material,exposure_class,creep_number,width,bar_diameter):
        '''Args:
        ...
        self.Ec_middle = self.calculate_E_middle(material.Ecm,creep_number.phi_self,creep_number.phi_self,load.M_SLS,load.Mg_SLS,load.Mp_SLS)
        self.alpha = self.calculate_alpha(material.Es,self.Ec_middle,cross_section.As,width,cross_section.d)
        self.sigma = self.calculate_reinforcement_stress(self.alpha,width,cross_section.d,load.M_SLS,self.Ec_middle,material.Es,cross_section.As)
    ...
    def calculate_reinforcement_stress(self,alpha: float, width: float, d: float, M_Ed: float, Ec_middle: float, 
        ...
        ''' Function that calculates reinforcement stress, according to (5.55) in Sørensen
    ...
    def calculate_maximal_bar_diameter(self,w_max:float , a_max: float)-> float:
        ...
        Args:
            ...
            a_max(float):  reinforcement stress [N/mm2]
        Returns:
            max_bar_diameter(float):  maximum bar diameter to limit crack width [mm]
        '''
