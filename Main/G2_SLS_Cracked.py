...
class Cracked_Stress:
    ...
    def __init__(self, material, cross_section, load, deflection, time_effect, creep_number):
        ...
        self.netta = material.Ep / self.Ec_middle
        self.ro_l = cross_section.Ap / (cross_section.width * cross_section.d_2)
        ...
        self.alpha = self.calculate_alpha(cross_section.d_2,cross_section.e,self.a)
        self.sigma_c_cracked = self.calculate_concrete_stress_cracked(cross_section.d_2,cross_section.width,self.alpha)
    ...
    def calculate_alpha(self, d: float, e: float, a: float)-> float:
        ...
        Returns:
            alpha(float):  factor
        '''
        coefficients = [d / (6 * (e + a)), 0.5 * (1 - d / (e + a)), self.netta * self.ro_l, - self.netta * self.ro_l] 
    ...
    def calculate_concrete_stress_cracked(self, d: float, width: float,alpha: float)-> float:
        ...
        '''
        sigma_c_cracked = (self.N * 10 ** 3) / (width * d * (0.5 * alpha - self.netta * self.ro_l * (1 - alpha)/alpha))
