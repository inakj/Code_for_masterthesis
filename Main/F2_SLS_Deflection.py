...
''' This script contain the Deflection class that apply for prestressed reinforced cross section.
'''
...
class Deflection_prestressed:
    ...
    def calculate_EI_cracked(self, alpha: float, width: float, d: float, Ec_middle: float,
        ...
        EI_2 = Ec_middle * self.Ic2 + Ep * self.Ip2 # from Sørensen (5.8)
        return EI_2
    ...
    def calculate_M_cr(self, fctm: float, Ic1: float, netta: float, Ip1: float, h: float, alpha_uncracked: float, d: float)-> float:
        ...
        Args: 
            ...
            netta(float):  material stiffness ratio
            Ip1(float):  second moment of inertia for reinforced steel, for uncracked cross section [mm4]
    ...
    def calculate_eps_cd_0(self, cement_class: str, RH: int, fcm: int) -> float :
        '''Function that calculate nominal free shrinkage strain due to drying according to EC2 Annex B.(1). 
        Args:
            cement_class(str):  cement class, from Input class 
            RH(int):  relative humidity, from Input [%]
