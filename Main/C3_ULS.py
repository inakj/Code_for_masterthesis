...
class ULS_prestress_and_ordinary:
    ...
    def __init__(self, material, load, cross_section, time_effect, Asw):
        '''Args:
        ...
        self.alpha = self.calculate_alpha(material.eps_cu3,cross_section.Ap,material.Ep,material.fcd,self.eps_diff,cross_section.width,cross_section.d_2,material.fpd,material.lambda_factor,material.netta,material.fyd,cross_section.As)
        ...
        self.V_Rd = self.calc_shear_capacity(cross_section.d_2,cross_section.Ac,cross_section.width,cross_section.Ap,material.fcd,material.gamma_concrete,material.fck,load.P0,material.gamma_prestressed_reinforcement,time_effect.loss)
    ...
    def calculate_alpha(self, eps_cu3: float, Ap: float, Ep: float, fcd: float, eps_diff: float,
        ...
        Args:
            ...
            fyd(int):  tensile strength in ordinary reinforcement [N/mm2]
            As(float):  ordinary reinforcement area [mm2]

        Returns:
            alpha(float):  Compression-zone-height factor for nonprestressed cross section
        '''
        alpha_b = eps_cu3 / (eps_cu3 + fpd / Ep - eps_diff)
        Apb = (netta * lambda_factor * alpha_b * width * d * fcd + fyd * As)/ fpd 
        if Ap <= Apb: 
            alpha = (fpd * Ap - fyd * As)/ (netta * lambda_factor * fcd * width * d)
        elif Ap > Apb:
    ...
    def calculate_moment_capacity(self, alpha: float, fcd: float, width: float, d: float, lambda_factor: float,
        ...
        Returns: 
            M_Rd(float):  moment capacity [kNm]
        '''
        M_Rd = netta * lambda_factor * alpha * (1 - 0.5 * lambda_factor * alpha) * fcd * width * d ** 2 + fyd * As * (d - cnom) 
        return M_Rd *  10 ** -6
    ...
    def calc_shear_capacity(self, d:float, Ac: float, width: float, Ap: int, fcd: float, 
        ...
        N_Ed = abs(P0) * gamma_prestress * (1 - loss/100)
        self.sigma_cp = min(N_Ed / Ac, 0.2 * fcd)
        CRd_c = 0.18 / gamma_concrete
        k_1 = 0.15 #assumed compressive force 
        v_min = 0.035 * k ** (3/2) * fck ** (0.5)
        V_Rd_c = (CRd_c * k * (100 * ro_l * fck) ** (1/3) + k_1 * self.sigma_cp) * width * d
        V_Rd_min = (v_min + k_1 * self.sigma_cp) * width * d
    ...
    def control_V(self, V_Rd: float, V_ULS: float, Asw: float, d: float, fyd: float, fck: float, width: float, fcd: float)-> bool:
        ...
        if V_Rd >= V_ULS:
            ...
            if 0 < abs(self.sigma_cp) <= 0.25 * fcd:
                alpha_cw = 1 + abs(self.sigma_cp)/ fcd
            elif 0.25 < abs(self.sigma_cp) <= 0.5 * fcd:
                alpha_cw = 1.25
            else:
                alpha_cw = 2.5 * (1 - abs(self.sigma_cp) / fcd)
            v = 0.6 * (1 - fck / 250)
