...
# NB factor is percentage of load that is long lasting
# conservative: Only add prestress as load, else: add it to bending stiffness

# ONLY SLS

class Deflection:
    '''Class to contain deformation because of longterm creep and shrink. 
    ...
    Have used design values for load where this give conservative solutions, and characteristic values where this is 
    most conservative. 
    '''

    def __init__(self,cross_section,material,load,creep_number, factor: float, length: float,
                 RH: int, cement_class: str,width,height):
        '''Args:
        ...
        '''
        self.Ec_middle = self.calculate_E_middle(material.Ecm,creep_number.phi_self,creep_number.phi_live,load.M_SLS,load.Mg_SLS,load.Mp_SLS)
        self.netta = self.calculate_netta(material.Es,self.Ec_middle)
        self.ro = self.calculate_ro(cross_section.As,width,cross_section.d)
        self.alpha_uncracked = self.calculate_alpha_uncracked(self.netta,cross_section.Ac,height,cross_section.As,cross_section.d)
        self.EI_1 = self.calculate_EI_uncracked(width,height,self.alpha_uncracked,cross_section.As,cross_section.d,self.Ec_middle,material.Es)
        ...
        self.EI_2 = self.calculate_EI_cracked(self.alpha_cracked,width,cross_section.d,self.Ec_middle,cross_section.As,material.Es)
        self.deflection_cracked = self.calculate_deflection_cracked(length,load.g_k,load.p_k,factor,self.EI_2)
        self.M_cr = self.calculate_M_cr(material.fctm,self.Ic1,self.netta,self.Is1,height,self.alpha_cracked,cross_section.d)
        ...
        self.eps_cd = self.calculate_eps_cd(self.eps_cd0,cross_section.Ac,width,height)
        self.eps_ca = self.calculate_eps_ca(material.fck)
        self.eps_cs = self.calculate_eps_cs(self.eps_cd,self.eps_ca)
        self.K_s = self.calculate_curvature(self.eps_cs,self.netta,cross_section.As,cross_section.Ac,height,cross_section.d,width)
    ...
    def calculate_EI_uncracked(self,width: float, h: float, alpha: float, As: float, d: float, 
        ...
        self.Is1 = As * (d - alpha * d) ** 2 
        EI_1 = Ec_middle * self.Ic1 + Es * self.Is1 
        return EI_1
    ...
    def calculate_alpha_cracked(self,netta: float, ro: float)-> float:
        ...
        Args:
            As(float):  reinforcement area from cross section class [mm2]
            width(float):  widht of cross section, defined by user [mm]
            d(float):  effective height from cross section class [mm]
    ...
    def calculate_EI_cracked(self,alpha: float, width: float, d: float, Ec_middle: float,
        ...
        self.Is2 = As * ((1 - alpha) * d) ** 2
        EI_2 = Ec_middle * self.Ic2 + Es * self.Is2
        return EI_2
    ...
    def calculate_M_cr(self,fctm: float, Ic1: float, netta: float, Is1: float, h: float, alpha_uncracked: float, d: float)-> float:
        ...
        Returns:
            M_cr(float):  crack moment [kNm]
        '''
        M_cr = fctm * (Ic1 + netta * Is1) / (h - alpha_uncracked * d)
        return M_cr * 10 ** (-6)
    ...
    def calculate_deflection_tension_stiffening(self,M_cr: float, M_Ed: float, control: bool, deflection_shrinkage: float,
       ...
       Args:
        ...
        beta = 0.5 
        if control == True: 
            self.zeta = 1 - beta * (M_cr / M_Ed) ** 2
        elif control == False: 
            zeta = 0
        total_deflection  = self.zeta * (deflection_cracked + deflection_shrinkage) 
        total_deflection += (1 - self.zeta) * (deflection_uncracked + deflection_shrinkage) #deflection_shrinkage)
