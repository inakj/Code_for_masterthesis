...
class cracked_concrete_prestress:
    ...
    def __init__(self,material,cross_section,load,phi_1,phi_2,eps_cs,delta_sigma_pr):
        ...
        self.delta_sigma_pL = self.calc_change_in_stress_longtime(self.sigma_c,self.alpha,material.Ecm,eps_cs,material.Ep,delta_sigma_pr)
    ...
    def calc_E_middle(self,Ecm,phi_1,phi_2,e,Mg_SLS,Mp_SLS):
        EcL_1 = Ecm / (1 + phi_1)
        EcL_2 = Ecm / (1 + phi_2)
        self.Mp = - self.P0 * e * 10 ** -3
        M1 = Mg_SLS
        M2 = Mp_SLS
    ...
    def calc_a(self,Mg_SLS,Mp_SLS,e):
        self.N = self.P0 - self.Ns #kN
        M = Mg_SLS + Mp_SLS + self.Mp + (self.Ns * e) * 10 ** -6
        a = M/self.N
        return a
    
    # equation 6.24
    def calc_alpha(self,d,e):
        # Coefficients of the cubic equation ax^3 + bx^2 + cx + d = 0
        coefficients = [d / 6 * (e + self.a), 0.5 * (1 - d / (e + self.a)), self.netta * self.ro, - self.netta * self.ro] 
    ...
    # equation 6.22
    def calc_sigma_c(self,d,width):
        sigma_c = (self.N * 10 ** 3) / (width * d * (0.5 * self.alpha - self.netta * self.ro * (1 -self.alpha)/self.alpha))
        return sigma_c # Compression in concrete at top
    
    def calc_sigma_p(self,eps_cs,Ep):
        eps_c = self.sigma_c / self.E_middle # concrete strain top 
        self.delta_eps_p = eps_c * (1 - self.alpha) / self.alpha # strain at prestressing
    ...
    def calc_change_in_stress_longtime(self,Ecm,eps_cs,Ep,delta_sigma_pr):
        eps_ck = (1 - self.alpha) / self.alpha * (self.sigma_c / Ecm) #Short time strain in concrete near pretension
